...
{
    public class kyu5_Event_and_Delegate : EventArgs
    {
    ...
    {
        public delegate void ContactNotifyEventHandler(object source, kyu5_Event_and_Delegate args);
        public event ContactNotifyEventHandler ContactNotify;

        protected void OnContactNotify(string name)
        {
            ContactNotify?.Invoke(this, new kyu5_Event_and_Delegate() { Name = name });
        }

        public void CountMessages(List<string> peopleList)
        {
            var peopleCounted = new Dictionary<string, int>();

            foreach (string person in peopleList)
            {
                if (peopleCounted.ContainsKey(person) == false)
                {
                    peopleCounted.Add(person, 0);
                }

                peopleCounted[person]++;

                if (peopleCounted[person] % 3 == 0)
                {
    ...
    {
        public static void Send(object source, kyu5_Event_and_Delegate e)
        {
